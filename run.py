...
class Experiment:
    ...
    def init_embeddings(self):
        ...
        if self.args.encoder == "compgcn":
            self.rel_embeddings = nn.Embedding(d.rel_num, (int)(self.args.hiddens[0] / 2)).to(device)
        else:
        ...
        self.enh_ent_emb = self.ent_embeddings.weight.cpu().detach().numpy()
    ...
    def train_and_eval(self):
        ...
        # 定义参数列表和优化器
        if self.args.encoder == "none":
            params = nn.ParameterList([self.ent_embeddings.weight, self.rel_embeddings.weight] + (list(decoder.parameters())) )
        else:
        ...
        opt = optim.Adam(params, lr=self.args.lr)
        logger.info(params)
        logger.info(opt)
        ...
        for it in range(0, self.args.epoch): # 循环 epoch 次 
            ...
            with torch.no_grad():  # 关闭梯度计算
                ...
                if self.args.early_stop:
                    ...
                    self.test_result = self.evaluate(it, d.ill_test_idx, embeddings) # 不 early_stop 每次记录 result 最后保存的就是最后一个 epoch 的结果
    ...
    def train_1_epoch(self, it, opt, encoder, decoder, edges, values, rels, triples, ills, ids, ent_emb, rel_emb):
        ...
        if decoder.name not in self.cached_sample or it % self.args.update == 0:
            ...
            if decoder.name in ["align"]:
                self.cached_sample[decoder.name] = ills.tolist() + d.boot_pair_dix
                self.cached_sample[decoder.name] = np.array(self.cached_sample[decoder.name])
            else:
                self.cached_sample[decoder.name] = triples + d.boot_triple_idx
        ...
        for i in range(0, len(train), train_batch_size):
            ...
            if self.args.encoder == "none":
                enh_emb = ent_emb
            elif self.args.encoder == "compgcn":
                enh_emb = encoder.forward(edges=use_edges, rels=use_rels, x=ent_emb, r=rel_emb)
            else:
                enh_emb = encoder.forward(edges=use_edges, rels=use_rels, x=ent_emb, r=rel_emb[d.sparse_rels_idx])
            # 更新增强实体嵌入 enh_ent_emb
            self.enh_ent_emb =  enh_emb.cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir", type=str, default="data/HGCNdata/zh_en", required=False, help="input dataset file directory, ('data/DBP15K/zh_en', 'data/SRPRS/en_fr_15k_V1')")
    parser.add_argument("--train_rate", type=float, default=0.3, help="training set rate")
    parser.add_argument("--val_rate", type=float, default=0, help="valid set rate")
    ...
    parser.add_argument("--heads", type=str, default="1,1,1,1,1", help="heads in each gat layer, splitted with comma")
    ...
    parser.add_argument('--activation', type=str, default='none', choices=['none', 'elu', 'relu', 'tanh', 'sigmoid'], help="activation function")
    ...
    args.epoch = 1000 # number of epochs to train
    args.update = 10 # number of epoch for updating negtive samples
    args.early_stop = True # whether to use early stop 
    args.patience = 0 # early stop patience epoch
